...
public class FillLineGraphicView  extends View{
    ...
    public FillLineGraphicView(Context context) {
        super(context);
        init(context);
    }

    public FillLineGraphicView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
                    ...
                    if (j == list.size() - 1) {
                      ...
                      //  canvas.drawText(value + "",startp.x + textLength / 2, startp.y - 10, titlePaint);
                       /* mPath.lineTo(getXY()[0],boot);
                        ...
                        if(j!=-1){
                           // canvas.drawCircle(startp.x, startp.y, 13, titlePaint);
                            //测量字符串宽度
                            float textLength = titlePaint.measureText(value + "");
                            //添加 点上文字
                          //  canvas.drawText(value + "",startp.x + textLength / 2, startp.y - 10, titlePaint);
                        }
                        mPath.lineTo(endp.x,boot);
                        mPath.lineTo(startp.x,boot);
    ...
    public int[] getXY() {
        ...
        xy[0] = date.get(0).size() * rectWidth-rectWidth;
        xy[1] = boot + 20;
        return xy;
    }
    ...
    public void getPoints() {
        for (int i = 0; i < date.size(); i++) {
            ...
            for (int j = 0; j < list.size(); j++) {
                ...
                Rect rect = new Rect();
                rect.left =rectWidth * j + leftPading;
//                  当前的相对高度：
